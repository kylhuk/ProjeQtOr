...
function refreshJsonPlanning(versionsPlanning) {
  ...
  if(needTimelineRefresh) callback = function(){ 
    ...
    if (valuePred!='' || valueSucc!='') drawPredecessorsAndSuccessors();
    refreshTimeline();};
  jsonData = (dojo.byId("workPlan"))?'workPlanJsonData':'planningJsonData';
...
function getJsonPlanningUrl (versionsPlanning) {
  ...
  if (dijit.byId('listShowProject') && versionsPlanning == 'workPlan') {
    ...
    url+=(param) ? "&" : "?";
    url+=(wpShowProject)?"showProject=true":"showProject=false";
    param=true;
  }
  ...
  if (dijit.byId('listShowProjectColor') && versionsPlanning == 'workPlan') {
    ...
    url+=(param) ? "&" : "?";
    url+=(wpShowProjectColor)?"showProjectColor=true":"showProjectColor=false";
    param=true;
  }
  ...
  if (dijit.byId('listShowLateColor') && versionsPlanning == 'workPlan') {
    ...
    url+=(param) ? "&" : "?";
    url+=(wpShowLateColor)?"showLateColor=true":"showLateColor=false";
    param=true;
  }
  ...
  if (dijit.byId('listShowPoolForResource') && versionsPlanning == 'workPlan') {
    ...
    url+=(param) ? "&" : "?";
    url+=(wpShowPoolForResource)?"showPoolForResource=true":"showPoolForResource=false";
    param=true;
  }
  ...
  if (dijit.byId('listShowResourceWithoutWork') && versionsPlanning == 'workPlan') {
    ...
    url+=(param) ? "&" : "?";
    url+=(wpShowResourceWithoutWork)?"showResourceWithoutWork=true":"showResourceWithoutWork=false";
    param=true;
  }
  ...
  if(dojo.byId("planningJsonData") && versionsPlanning == 'workPlan'){
    var vTaskList=g.getList();
    var startDateView = JSGantt.formatDateStr(JSGantt.getMinDate(vTaskList, vFormat, g.getStartDateView()),'yyyy-mm-dd');
    var endDateView = JSGantt.formatDateStr(JSGantt.getMaxDate(vTaskList, vFormat, g.getEndDateView()),'yyyy-mm-dd');
    url+=(param) ? "&" : "?";
    url+="planningStartDateView="+startDateView;
    url+="&planningEndDateView="+endDateView;
    param=true;
  }
...
function refreshGrid(noReplan) {
  ...
  if (dijit.byId("objectGrid")) { // Grid exists : refresh it
    ...
    else refreshJsonList(dojo.byId('objectClass').value,true);  } else { // If Grid does not exist, we are displaying Planning : refresh it
    showWait();
...
//	  if(grid.store.getValue(item,fldSum)==""){
//	    sumValue+=0;
//	  }else{
//	    sumValue+=parseFloat(grid.store.getValue(item,fldSum));
//	  }
//	});
	sumValue=0;
	nameFilter='*';
	...
	var filterId=dojo.byId('listIdFilter');
  var filterName=dojo.byId('listNameFilter');
  if (filterName && filterName.value) nameFilter='*'+filterName.value+'*';
  if (filterId && filterId.value) idFilter='*'+filterId.value+'*';
  grid.store.fetch({
	  query: { name: nameFilter, id: idFilter },	  
	  queryOptions: { ignoreCase: true },
	  onComplete: function (items) {
	    dojo.forEach(items, function (item, index) {
	      if (item.summable==undefined || item.summable==1) sumValue+=parseFloat(grid.store.getValue(item,fldSum));
	    });
	  }
	});
...
function selectPlanningRow(autoscroll) {
  setTimeout("selectPlanningLine(dojo.byId('objectClass').value,dojo.byId('objectId').value,"+((autoscroll===true)?'true':'false')+");",1);
}
...
function setSelectedProject(idProject,nameProject,selectionField,resetPrevious,goToProject,objectClass,objectId) {
  ...
  if (idProject != "") {
    var callBack=function() {
      ...
      var gotoPlanningCallBack=function() {
        if(gotoPlanning){
          noRefresh=true;
          if (!objectClass) objectClass = dojo.byId('objectClassRow').value;
          if (!objectId) objectId = dojo.byId('objectIdRow').value;
          gotoElement(objectClass,objectId,false, false,'planning',false);
        }
      }
      saveDataToSession('projectSelected',idProject,true,gotoPlanningCallBack);
    };
...
//  arrayCollapsed[scope]='true';
  var callXhrPost=function(intermediate) {
    dojo.xhrPost({
      url : "../tool/saveCollapsed.php?scope=" + collapsedTrue + "&value=true"+top.addTokenIndexToUrl(),
      handleAs : "text",
      load : function(data, args) {
        if (! intermediate) {
          collapsedTrue="";
...
//  arrayCollapsed[scope]='false';
  var callXhrPost=function(intermediate) {
    dojo.xhrPost({
      url : "../tool/saveCollapsed.php?scope=" + collapsedFalse + "&value=false"+top.addTokenIndexToUrl(),
      handleAs : "text",
      load : function(data, args) {
        if (! intermediate) {
          collapsedFalse="";
...
function setDisplaySM(){
  dojo.query('.loginFrameNewGui').forEach(function(node){
    var parent = node.parentNode;
    var inputSM = dojo.create('div', {
      id: 'msgSb',
      style:'position:fixed;bottom:0px;right:10px;padding:5px;font-size:10pt;color:var(--color-light-secondary);'
    }, parent);
...
function ckEditorReplaceEditor(editorName,numEditor) {
  ...
  if (editorName != 'noteNote' && editorName != 'WUDescriptions' && editorName != 'WUIncomings' && editorName != 'WULivrables' && editorName != "situationComment" && editorName != "eventDescription" && editorName != "voteNote") { // No
    // formChanged
    // for
    // notes
...
function handleResizeGantBar(element,refId,id,minDate,dayWidth,dateFormat,pm) {
  ...
  function initDragStart(e) {
    ...
    while(calcDate <= calcDateEnd){
      ...
      calcStartDate=calcStartDate + (24 * 60 * 60 * 1000);
...
function updateSubTask(id,refType,refId,isPrio,isRes) {
  ...
  //console.log('id : ',id, 'dueDate : ',dueDateVal);
...
function reportGotoElement(idResource, date, refType, refId, idProject, event, nameProject){
  ...
  reportExtraInformations(idResource, date, event);
}

var currentOpenedDetailDiv = null;
ongoingReportExtraInformations=false;
function reportExtraInformations(idResource, date, event) {
  if (ongoingReportExtraInformations) return;
  var currentDivId = "colorPlanDetailDiv-" + idResource + "-" + date.replace(/-/g, '');
  ...
  dojo.xhrGet({
    url: "../report/colorPlanDetail.php?idResource=" + idResource + "&date=" + date,
    load: function(data, args) {
      ...
      detail.style.left = mouseX - 300 +'px';
      if ( mouseY < (screen.height - detail.offsetHeight - 150) ) detail.style.top = mouseY + 30 + 'px';
      else detail.style.top = mouseY - detail.offsetHeight - 10 + 'px';
      ...
      if (closeButton) {
        ...
        closeButton.style.top = (detail.offsetTop - 20 ) + 'px';
        closeButton.style.left = (detail.offsetLeft + detail.offsetWidth - 27 ) + 'px';
...
function showTooltip(element, project, object, resource, productVersion, type, planningMode) {
...
function addTokenIndexToUrl(urlParam) {
  ...
  urlParamExt='';
  if (typeof csrfToken=='undefined' || csrfToken==undefined || ! csrfToken) { csrfTokenParam=''; }
  else { csrfTokenParam=csrfToken; }
  if (urlParam.indexOf("csrfToken=") < 0) {
    urlParamExt+=((paramAddStart=='no')?'&':'?')+"csrfToken="+csrfTokenParam;
    paramAddStart='no';
  }
  
  if (typeof directAccessIndex=='undefined' || directAccessIndex==undefined || ! directAccessIndex) { directAccessIndexParam=''; } 
  else { directAccessIndexParam=directAccessIndex; }
