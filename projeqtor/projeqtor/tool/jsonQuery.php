...
// END ADD BY Marc TABARY - 2017-02-20 - ORGANIZATION VISIBILITY            
    ...
    if ($outMode=='csv') {
      ...
      if ($sumFld and $socSw=='on') {
        $from='';
        $format='';
        if (property_exists($objectClass,$sumFld)){ $from='';}
    			...
    			foreach ($line as $id => $val) {
    			  ...
    			  if ($id=='refType') $refType=$val;
                    ...
                    if ( ( isset($dataType[$id]) and $dataType[$id]=='varchar' or $foreign) and $csvQuotedText) { 
                      ...
                      if ( ($objectClass == 'Work' and Parameter::getGlobalParameter('imputationUnit')=='hours' and $id=='work') 
                      or   ($objectClass == 'Ticket' and property_exists($obj, 'WorkElement') and property_exists('WorkElement', $colId) and Parameter::getGlobalParameter('imputationUnit')=='hours' and pq_substr($colId,-4)=='Work') ) {
