...
$idFilterCriteria=SqlList::getIdFromName("filter", $filterActivity);
...
if($idFilterCriteria){
  ...
  if($startDateReport<$today){
    ...
    echo i18n('incorrectDateToEarlyFilterActivity',array(i18n('colStartDate'))); // TODO i18n message
    echo '</div>';
...
if($idFilterCriteria){
//join
  $queryJoin=  " JOIN $actcrtTable a ON pw.refId = a.id  AND pw.refType = 'Activity'";
  if($peCritOn){
    //join for planningElement
   $queryJoin .= "JOIN $peTable pe ON pw.refId = pe.refId AND pe.refType = 'Activity'";
  }
}
//where
...
if(!$idProject){
  $pe=SqlElement::getSingleSqlElementFromCriteria('PlanningElement',array('refType'=>'Project'));
}else{
